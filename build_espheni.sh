...
do
    case $opt in
        ...
        "kltekdi")
			...
			rm -r -f output
			mkdir output
			make -C $(pwd) O=output espheni_msm8974_defconfig VARIANT_DEFCONFIG=espheni_msm8974pro_kltekdi_defconfig SELINUX_DEFCONFIG=espheni_selinux_permissive_defconfig TIMA_DEFCONFIG=espheni_tima_defconfig
			make -j64 -C $(pwd) O=output
			cp output/arch/arm/boot/zImage $(pwd)/arch/arm/boot/zImage
            echo " "
            echo "------------------------------------------"
        ...
        "kltevzw")
			...
			rm -r -f output
			mkdir output
			make -C $(pwd) O=output espheni_msm8974_defconfig VARIANT_DEFCONFIG=espheni_msm8974pro_kltevzw_defconfig SELINUX_DEFCONFIG=espheni_selinux_permissive_defconfig TIMA_DEFCONFIG=espheni_tima_defconfig
			make -C $(pwd) O=output
			cp output/arch/arm/boot/zImage $(pwd)/arch/arm/boot/zImage
            echo " "
            echo "------------------------------------------"
